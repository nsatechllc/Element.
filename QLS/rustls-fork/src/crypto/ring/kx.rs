...
// PQ-only Kyber group integration (experimental)
use rustls_kyber;
...
/// Experimental Kyber768 KEM-only key exchange group.
/// start() generates a Kyber keypair; complete() is not used (server path overrides with encapsulation logic),
/// but we implement it to return an error if invoked improperly.
#[derive(Debug)]
struct KyberKxGroup;

impl SupportedKxGroup for KyberKxGroup {
    fn start(&self) -> Result<Box<dyn ActiveKeyExchange>, Error> {
        let kem = rustls_kyber::KyberKem::new().map_err(|_| GetRandomFailed)?;
        let kp = kem.keypair().map_err(|_| GetRandomFailed)?;
    Ok(Box::new(KyberActive { public: kp.public.clone(), secret: kp.secret.clone() }))
    }
    fn name(&self) -> NamedGroup { NamedGroup::Kyber768Exp }
    fn fips(&self) -> bool { false }
}

pub(crate) struct KyberActive { pub(crate) public: Vec<u8>, pub(crate) secret: Vec<u8> }
impl KyberActive { pub(crate) fn secret_bytes(&self) -> &[u8] { &self.secret } }
impl ActiveKeyExchange for KyberActive {
    fn as_any(&self) -> &dyn core::any::Any { self }
    fn complete(self: Box<Self>, _peer_pub: &[u8]) -> Result<SharedSecret, Error> {
        // In Kyber-only mode the server supplies ciphertext; classical path should not call this.
        Err(PeerMisbehaved::InvalidKeyShare.into())
    }
    fn group(&self) -> NamedGroup { NamedGroup::Kyber768Exp }
    fn pub_key(&self) -> &[u8] { &self.public }
}

pub(crate) static KYBER768_EXP: &dyn SupportedKxGroup = &KyberKxGroup;
