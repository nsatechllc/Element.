...
// PQC integration crate
use rustls_kyber; // ensure linkage
...
struct ExpectServerHello {
    ...
    // Experimental: Kyber keypair secret for PQ key share (if generated)
    kyber_secret: Option<Vec<u8>>,
}
...
fn emit_client_hello_for_retry(
    ...
    let mut exts = Box::new(ClientExtensions {
        ...
        named_groups: {
            let mut groups: Vec<_> = config
                ...
                .map(|skxg| skxg.name())
                .collect();
            // Prepend experimental hybrid/PQ groups if classical X25519 is present to signal preference.
            if groups.iter().any(|g| matches!(g, NamedGroup::X25519)) {
                // Avoid duplicates if provider later embeds them.
                use NamedGroup::{X25519Kyber768HybridExp, Kyber768Exp};
                if !groups.contains(&X25519Kyber768HybridExp) { groups.insert(0, X25519Kyber768HybridExp); }
                if !groups.contains(&Kyber768Exp) { groups.insert(1, Kyber768Exp); }
            }
            Some(groups)
        },
    ...
    // Temporary store for Kyber secret key if generated.
    let mut kyber_secret_generated: Option<Vec<u8>> = None;
    ...
    if let Some(key_share) = &key_share {
        ...
        // Optionally append a Kyber keyshare if we advertised Kyber groups.
        // For now we unconditionally generate if Kyber768Exp was placed in named_groups list.
        // This is a first step; future refinement may tie this to hybrid negotiation decisions.
        if let Some(named_groups) = &exts.named_groups {
            // Only generate if we are not already offering a Kyber key share (avoid duplicates)
            let already_has_kyber = shares.iter().any(|s| s.group == NamedGroup::Kyber768Exp);
            if !already_has_kyber && named_groups.iter().any(|g| matches!(g, NamedGroup::Kyber768Exp)) {
                if let Ok(kem) = rustls_kyber::KyberKem::new() {
                    if let Ok(kp) = kem.keypair() {
                        shares.push(KeyShareEntry::new(NamedGroup::Kyber768Exp, kp.public.clone()));
                        kyber_secret_generated = Some(kp.secret.clone());
                    }
                }
            }
        }
    ...
    // kyber_secret_generated will be passed through state -> tls13 handler directly.
    ...
    let next = ExpectServerHello {
        ...
        suite,
        ech_state,
        kyber_secret: kyber_secret_generated,
    };
