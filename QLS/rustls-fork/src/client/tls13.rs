...
pub(super) fn handle_server_hello(
    ...
    // (Temporarily deferred until after we unpack ClientHelloInput)
    ...
    // Downgrade detection (simplified): if PQ required and we started with Kyber as initial kx_state hint
    // but server selected a different group.
    if config.require_pq_kem
        && their_key_share.group != crate::NamedGroup::Kyber768Exp
    {
        if let KxState::Start(g) = cx.common.kx_state {
            if g.name() == crate::NamedGroup::Kyber768Exp {
                return Err(cx.common.send_fatal_alert(
                    AlertDescription::IllegalParameter,
                    PeerMisbehaved::PqDowngradeDetected,
                ));
            }
        }
    }
    ...
    let shared_secret;
    if their_key_share.group == crate::NamedGroup::Kyber768Exp {
        // Downcast to access secret; do not consume our_key_share yet.
        if let KeyExchangeChoice::Whole(ref akx) = our_key_share {
            if let Some(kact) = akx.as_any().downcast_ref::<crypto::ring::kx::KyberActive>() {
                if let Ok(kem) = rustls_kyber::KyberKem::new() {
                    match kem.decapsulate(kact.secret_bytes(), &their_key_share.payload.0) {
                        Ok(ss) => { shared_secret = SharedSecret::from(ss.as_bytes()); }
                        Err(_) => { return Err(Error::DecryptError); }
                    }
                } else { return Err(Error::DecryptError); }
            } else { return Err(Error::DecryptError); }
        } else { return Err(Error::DecryptError); }
    } else {
    shared_secret = our_key_share
