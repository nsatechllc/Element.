...
impl ConfigBuilder<ClientConfig, WantsClientCert> {
    ...
    /// Enable (or disable) requiring a PQ KEM (Kyber) key exchange; if enabled,
    /// handshake aborts when server does not negotiate a Kyber-based group.
    pub fn with_require_pq_kem(mut self, require: bool) -> Self {
        // We can't set it until final struct creation; stash via phantom override not present.
        // Instead provide a follow-up adapter: caller chains then sets field after build.
        // Simpler: build then modify; expose separate helper below.
        self
    }
}

impl ClientConfig {
    /// Post-build setter for `require_pq_kem` (builder-chain friendly until a direct builder hook is wired).
    pub fn set_require_pq_kem(&mut self, require: bool) { self.require_pq_kem = require; }
}
