...
mod client_hello {
    ...
    fn emit_server_hello(
        ...
        // If Kyber768Exp selected, perform KEM encapsulation instead of classical start_and_complete.
    let ckx = if kxgroup.name() == NamedGroup::Kyber768Exp {
            // share.payload.0 holds client's Kyber public key.
            match rustls_kyber::KyberKem::new()
                .and_then(|kem| kem.encapsulate(&share.payload.0).map(|(ct, ss)| (ct, ss)))
            {
                Ok((ciphertext, ss)) => {
                    use crate::crypto::SharedSecret as TlsSharedSecret;
                    crate::crypto::CompletedKeyExchange {
                        group: NamedGroup::Kyber768Exp,
                        pub_key: ciphertext, // server sends ciphertext as its keyshare payload
                        secret: TlsSharedSecret::from(ss.as_bytes()),
                    }
                }
                Err(_) => {
                    return Err(cx
